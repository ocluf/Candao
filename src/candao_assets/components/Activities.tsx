...
export default function Activities() {
    ...
    const Activities: Array<ActivityCard> = [
        { name: "Total members", subtitle: nrOfMembers(), route: "/members", icon: <UsersIcon className="relative top-3 left-3 bg-purple-600 h-5 w-5 text-white" /> },
        { name: "Total canisters", subtitle: "3", route: "/canisters", icon: <ServerIcon className="relative top-3 left-3 bg-purple-600 h-5 w-5 text-white" /> },
        { name: "Proposals", subtitle: "20 new", route: "/proposals", icon: <CursorClickIcon className="relative top-3 left-3 bg-purple-600 h-5 w-5 text-white" /> }
    ];
    return (
        <main className="px-5">
            <h1 className="py-5 text-lg text-grey-900 font-medium leading-6">Activities</h1>
                                ...
                                <div className="w-full pl-4 flex items-center justify-between space-x-6">
                                    <div className="flex-1 truncate">
                                        <div className="flex items-center">
                                            <h3 className=" text-gray-500 text-sm truncate">{activity.name}</h3>
                                        </div>
                                        <p className=" text-gray-900 text-2xl font-medium truncate">{activity.subtitle}</p>
                                    </div>
